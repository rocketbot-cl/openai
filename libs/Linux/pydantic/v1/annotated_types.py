...
if TYPE_CHECKING:
    from r_typing_extensions import TypedDict
...
def create_model_from_typeddict(
    ...
    **kwargs: Any,
    ...
    # Best case scenario: with python 3.9+ or when `TypedDict` is imported from `r_typing_extensions`
    if not hasattr(typeddict_cls, '__required_keys__'):
        raise TypeError(
            'You should use `r_typing_extensions.TypedDict` instead of `typing.TypedDict` with Python < 3.9.2. '
    ...
    if is_legacy_typeddict(typeddict_cls) and any(
        ...
        raise TypeError(
            'You should use `r_typing_extensions.TypedDict` instead of `typing.TypedDict` with Python < 3.11. '
