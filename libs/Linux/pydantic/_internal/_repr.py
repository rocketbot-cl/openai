...
if typing.TYPE_CHECKING:
    ReprArgs: r_typing_extensions.TypeAlias = 'typing.Iterable[tuple[str | None, Any]]'
    RichReprResult: r_typing_extensions.TypeAlias = (
...
def display_as_type(obj: Any) -> str:
    ...
    if is_union_origin(r_typing_extensions.get_origin(obj)):
        args = ', '.join(map(display_as_type, r_typing_extensions.get_args(obj)))
        return f'Union[{args}]'
    elif isinstance(obj, _typing_extra.WithArgsTypes):
        if typing_objects.is_literal(r_typing_extensions.get_origin(obj)):
            args = ', '.join(map(repr, r_typing_extensions.get_args(obj)))
        else:
            args = ', '.join(map(display_as_type, r_typing_extensions.get_args(obj)))
        try:
            return f'{obj.__qualname__}[{args}]'
        except AttributeError:
            return str(obj).replace('typing.', '').replace('r_typing_extensions.', '')  # handles TypeAliasType in 3.12
    elif isinstance(obj, type):
        return obj.__qualname__
    else:
        return repr(obj).replace('typing.', '').replace('r_typing_extensions.', '')
