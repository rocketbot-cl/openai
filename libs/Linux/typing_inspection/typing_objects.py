...
The provided functions in this module check against both the [`typing`][] and [`r_typing_extensions`][]
...
import r_typing_extensions
from r_typing_extensions import LiteralString, TypeAliasType, TypeIs, deprecated
...
def _compile_identity_check_function(member: LiteralString, function_name: LiteralString) -> FunctionType:
    ...
    The function will make sure to check against both the `typing` and `r_typing_extensions`
    variants as depending on the Python version, the `r_typing_extensions` variant might be different.
    For instance, on Python 3.9:

    ```pycon
    >>> from typing import Literal as t_Literal
    >>> from r_typing_extensions import Literal as te_Literal, get_origin
    ...
    r_typing_extensions.Literal
    ```
    """
    in_typing = hasattr(typing, member)
    in_r_typing_extensions = hasattr(r_typing_extensions, member)

    if in_typing and in_r_typing_extensions:
        if getattr(typing, member) is getattr(r_typing_extensions, member):
            check_code = f'obj is typing.{member}'
        else:
            check_code = f'obj is typing.{member} or obj is r_typing_extensions.{member}'
    elif in_typing and not in_r_typing_extensions:
        check_code = f'obj is typing.{member}'
    elif not in_typing and in_r_typing_extensions:
        check_code = f'obj is r_typing_extensions.{member}'
    else:
        check_code = 'False'
    ...
    globals_: dict[str, Any] = {'Any': Any, 'typing': typing, 'r_typing_extensions': r_typing_extensions}
...
def _compile_isinstance_check_function(member: LiteralString, function_name: LiteralString) -> FunctionType:
    ...
    if in_typing and in_r_typing_extensions:
        if getattr(typing, member) is getattr(r_typing_extensions, member):
            ...
            check_code = f'isinstance(obj, (typing.{member}, r_typing_extensions.{member}))'
    elif in_typing and not in_r_typing_extensions:
        check_code = f'isinstance(obj, typing.{member})'
    elif not in_typing and in_r_typing_extensions:
        check_code = f'isinstance(obj, r_typing_extensions.{member})'
    else:
        check_code = 'False'
...
if _IS_PY310:
    # Parameterized PEP 695 type aliases are instances of `types.GenericAlias` in r_typing_extensions>=4.13.0.
...
if sys.version_info >= (3, 13):
    ...
    def is_deprecated(obj: Any, /) -> 'TypeIs[deprecated]':
        return isinstance(obj, (warnings.deprecated, r_typing_extensions.deprecated))

else:

    def is_deprecated(obj: Any, /) -> 'TypeIs[deprecated]':
        return isinstance(obj, r_typing_extensions.deprecated)


is_deprecated.__doc__ = """
Return whether the argument is a [`deprecated`][warnings.deprecated] instance.

This also includes the [`r_typing_extensions` backport][r_typing_extensions.deprecated].

```pycon
>>> is_deprecated(warnings.deprecated('message'))
True
>>> is_deprecated(r_typing_extensions.deprecated('message'))
True
```
"""
...
# Add the `r_typing_extensions` aliases:
for alias, target in list(DEPRECATED_ALIASES.items()):
    # Use `alias.__name__` when we drop support for Python 3.9
    if (te_alias := getattr(r_typing_extensions, alias._name, None)) is not None:
